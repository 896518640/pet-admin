...
router.post('/like/parent', async(req, res) => {
  ...
  const doc = await messageDB.findById(id)
  // 检测doc是否存在
  if (!doc) {
    return res.send({
      code: 1,
...
router.post('/like/child', async(req, res) => {
  ...
  const { pid, cid, cindex } = req.body

  const parent = await messageDB.findById(pid)
  // 验证数据
  if (!parent) {
    return res.send({
      code: 1,
  ...
  const child = parent.children.id(cid)
  if (!child) {
    return res.send({
      code: 1,
      msg: '子评论id错误'
...
router.post('/reply/submit', async(req, res) => {
  const { text, replyUser, id } = req.body
  // 鉴权
  if (!req.session.userInfo) {
    return res.send({
      code: 6,
  ...
  // 验证用户id
  const doc2 = await userDB.findById(replyUser)
  if (!doc2) {
    return res.send({
      code: 3,
      msg: '回复的用户id错误'
